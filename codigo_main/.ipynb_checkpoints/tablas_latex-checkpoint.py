import pandas as pd
from codigo_main_jupyter import num_def_cancer as mc
from codigo_main_jupyter import tmb_cancer as tasas
from codigo_main_jupyter import igualar_tablas as tb
from tabulate import tabulate
import os
from codigo_main_jupyter import formato_tabla as tb2
        ...
        if tabular == 1 or tabular ==2:
            if sexo == 0:
                ...
                if tabular == 1:
                    ...
                    tabla2.append(tabla)
                    tabla_final = tabulate(tabla2, headers=['Causa', 'Nº Defunciones', 'Tasa Mortalidad Bruta'], tablefmt="latex")
                elif tabular == 2:
                    tabla = list()
                    tabla2 = list()
                    for a, i in num_muertes.items():
                ...
                if sexo == 1:
                    ...
                    tabla.append(round(tasa, 2))
                    tabla2.append(tabla)
                    tabla_final = tabulate(tabla2, headers=['\nCausa', 'Nº Defunciones\nMujeres', 'Tasa Mortalidad Bruta'], tablefmt="latex")
                elif sexo == 3:
            ...
            if sexo == 0:
                ...
                for valor in tasa.values():
                    ...
                    i += 1
                tabla_final = tabulate(tabla, headers = ['Causa', 'Nº Defunciones', 'Tasa de Mortalidad Bruta'], tablefmt="latex")
            elif sexo != 0:
                if sexo == 1:
                    num_muertes = mc.num_muertes_cancer(df, per, tab, reg, com, sex,
                                                        edad_quinquenal=False)
        ...
        if tabular == 1 or tabular ==2:
            if sexo == 0:
                ...
                if tabular == 1:
                    ...
                    for value in tasa.values():
                        ...
                        i += 1
                    tabla_final = tabulate(tabla, headers=['Grupo edad', 'Causa', 'Nº Defunciones', 'Tasa Mortalidad Bruta'], tablefmt="latex")
                elif tabular == 2:
                    tabla = list()
                    for key, value in num_muertes.items():
                        a = list()
            ...
            if sexo == 0:
                ...
                for tabla in tabla_maxima:
                    tablas_causas = tabulate(tabla, tablefmt="latex", headers=(["Grupo de edad", "Causa", "Número\nDefunciones", "Tasa Mortalidad\nBruta" ]))
                    print(tablas_causas)
                ...
                if sexo == 1:
                    ...
                    tabla_maxima = tb2.formato_tabla(datos1, datos2, tabular)
                    for tabla in tabla_maxima:
                        tablas_causas = tabulate(tabla, tablefmt="latex", headers=(
